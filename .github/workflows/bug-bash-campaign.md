...
1. **Determine the project to use:**
   - **REQUIRED**: Use the exact project URL from `${{ inputs.project_url }}` in all `update-project` safe outputs
   - The project URL must be provided as input - this workflow does not support automatic project selection by name
   - Use the URL exactly as provided without modification
...
{
  "type": "update-project",
  "project": "https://github.com/users/monalisa/projects/42",
  "content_type": "issue",
  "content_number": 123,
  "fields": {
    "Status": "Todo",
...
**Important:** The `project` field must be a **project URL** (e.g., "https://github.com/users/monalisa/projects/42"). Use the exact URL provided in the workflow input without modification.
